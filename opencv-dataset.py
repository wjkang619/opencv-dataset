...
def parse_args():
	...
	parser.add_argument('-p','--process_type', type=str,
		default='resize',
		help='Process to use. ["resize","resize_square","square","crop_to_square","canny","canny-pix2pix","crop_square_patch","scale","many_squares","crop","distance"] (default: %(default)s)')
...
def makeResizeSquare(img,filename,scale):
	...
	if(args.force_max):
		...
		if(diff_x%2 == 0 and diff_y%2 == 0 ):
			img_copy= cv2.copyMakeBorder(img_copy, int(diff_y/2), int(diff_y/2), int(diff_x/2), int(diff_x/2), bType,value=bColor)
		elif(diff_x%2 == 0):
			img_copy = cv2.copyMakeBorder(img_copy, int(diff_y/2)+1, int(diff_y/2), int(diff_x/2), int(diff_x/2), bType,value=bColor)
		else:
			img_copy = cv2.copyMakeBorder(img_copy, int(diff_y/2), int(diff_y/2), int(diff_x/2)+1, int(diff_x/2), bType,value=bColor)
	elif(h > w):
		# pad left/right
		diff = h-w
		if(diff%2 == 0):
			img_copy = cv2.copyMakeBorder(img_copy, 0, 0, int(diff/2), int(diff/2), bType,value=bColor)
		else:
			img_copy = cv2.copyMakeBorder(img_copy, 0, 0, int(diff/2)+1, int(diff/2), bType,value=bColor)
	elif(w > h):
		# pad top/bottom
		diff = w-h

		if(args.v_align == 'bottom'):
			img_copy = cv2.copyMakeBorder(img_copy, diff, 0, 0, 0, bType,value=bColor)
		else:
			if(diff%2 == 0):
				img_copy = cv2.copyMakeBorder(img_copy, int(diff/2), int(diff/2), 0, 0, bType,value=bColor)
			else:
				img_copy = cv2.copyMakeBorder(img_copy, int(diff/2), int(diff/2)+1, 0, 0, bType,value=bColor)
	else:
		diff = scale-h
		if(diff%2 == 0):
			img_copy = cv2.copyMakeBorder(img_copy, int(diff/2), int(diff/2), int(diff/2), int(diff/2), bType,value=bColor)
		else:
			img_copy = cv2.copyMakeBorder(img_copy, int(diff/2), int(diff/2)+1, int(diff/2), int(diff/2)+1, bType,value=bColor)
...
def processImage(img,filename):
	...
	if args.process_type == "resize_square":
		makeResizeSquare(img,filename,args.max_size)
